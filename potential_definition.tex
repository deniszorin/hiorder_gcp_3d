...
If the closest point is on an edge ray, or on a face sector:
The condition to check is,  in this case,
...
\end{cases}
\]
If the closest point is at the vertex $v$, we consider the point $q' = 2p_v - q$,  reflected with respect to $p_v$.  As explained in the 
GCP paper, if $q$ is outside, then $q'$ has to be on the opposite side, 
and moreover, its closest point on $C$ cannot be $p_v$. 
Then the complete condition is:

\[
\mbox{Out}^{v}(q)=
\begin{cases}
\mbox{Out}^{v,fe}(q),\;\mbox{if $a(q) = f^*$ or $a(q) = e^*$,}\\
\bar{\mbox{Out}}^{v,fe}(q'),\mbox{if $a(q) = v$.}
\end{cases}
\]
\end{itemize}
\end{document}