...
If the closest point is at the vertex $v$, one can show that the $C$
and $q$ are on different sides of the plane $P$ with normal $q-p_v$ and passing through $p_v$.  Suppose an edge $e_j$ for some $j(q)$ is not perpendicular to $q-p_v$, then we can check if $p_j$ is on the opposite side of the plane $P$.  Otherwise, all faces incident at $v$
are in the plane $P$, and we can check whether $q$ is on the same side as any normal is pointing to.
...
(q - p_v) \cdot n_0 > 0, \mbox{if $a(q) = v$, and for all $j$
$ (q - p_v)  \cdot (p_{j(q)} - p_v) =0$  }
\end{cases}
\]
