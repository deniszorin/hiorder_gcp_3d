...
def smoothed_offset_potential(
    ...
    if include_faces or include_edges or include_vertices:
        ...
        for f in range(nf):
            ...
            for i in range(3):
                ...
                # project q to each edge
                q_to_origin = q - origin
                # M x 3 dotted with 3 vector row-wise  t is M 
                #  t = (q[j] - p[i]) dot d_e[i]     
                t = np.dot(q_to_origin, d_e)
                # M x 3   row P_e[j] = origin[j] + t 
                P_e = origin + np.outer(t, d_e)
                # M x 3 direction to the projection to the edge  
                vec = q - P_e 
                vec_norm = np.linalg.norm(vec, axis=1)
                unit = vec / np.maximum(vec_norm[:, None], eps)
                # Phi per face edge,  phi[j] = unit[j] dot inward
                #  phi = np.dot(unit, inward)
            ...
            if include_faces:
                ...
                I_f = np.where(r_f <= eps, singular_value, B / denom)
                face_sum += I_f
    ...
    if include_vertices:
        ...
        for v in range(nv):
            ...
            I_v = np.where(r_v <= eps, singular_value, B_vertex / denom)
            vertex_sum += I_v
